module.exports = angular.module("app.loginrxrx")
    .controller("loginrxrxCtrl", ['$scope', '$state', 'httpService', '$timeout', '$rootScope', 'apiUrl', 'Md5', 'ngDialog', function($scope, $state, httpService, $timeout, $rootScope, apiUrl, Md5, ngDialog) {
        ...
        $scope.login = function() {
			...
			$state.go('examinerx.starttrialrx');
            ...
            httpService.linkHttp(apiUrl.login, data).then(function(res) {
                ...
                if ($scope.data.errCode == "000000") {
                    if (!$scope.isRemember) { //记住账号
                        ...
                        window.sessionStorage.setItem('user', window.JSON.stringify($scope.data.data))
                            // $state.go('examine')
                   ...
                   /* var dataUA = {
					...
					httpService.linkHttp(apiUrl.addUserAgreement, dataUA).then(function(resUA) {
						...
						console.log(res);
						if (_dataUA.data.errCode == "000000") {
							 $state.go('examinerx.starttrial');
						}else{
							var dialog = ngDialog.open({
		                        template: '<p>' + _dataUA.data.errDesc + '</p>',
		                        plain: true,
        ...
        $scope.loginClick =function(){
            ...
            switch(true){
                case $scope.form.mobile == '':
                    $scope.userPro = true;
                break;
                case $scope.form.password == '':
                    $scope.pwdPro = true;
                break;
