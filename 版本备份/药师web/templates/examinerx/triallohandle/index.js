module.exports = angular.module("app.triallohandlerx")
    .controller("triallohandlerxCtrl", ['$scope', '$state', '$http','httpService', '$stateParams', '$timeout', '$interval','$rootScope', 'apiUrl', 'ngDialog', function($scope, $state, $http, httpService, $stateParams, $timeout, $interval,$rootScope,apiUrl,ngDialog) {
		...
		$scope.initTriall = function() {
			var time = $interval(function() {
				if($state.current.url != '/triallohandlerx'){
        	...
        	httpService.linkHttp(_urlOB,data).then(function(res) {
                ...
                if (rsdata.errCode == "000000") {
	                ...
	                ngDialog.openConfirm({
	                    ...
	                    console.log($scope.orderNo);
						ngDialog.close();
						$state.go('examinerx.prescriptionauditrx',{'orderNo':$scope.orderNo});
	                }, function (reason) {
	                    console.log('Modal promise rejected. Reason: ', reason);
	                    $scope.stopTrial();
	                });
